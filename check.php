...
// mysqli_close($conn);
   ...
   include_once 'config.php';  // Giả sử file config chứa kết nối cơ sở dữ liệu

   // Khởi tạo đối tượng DatabaseWrapper
   $database = new DatabaseWrapper($conn);
   ...
   if ($_SERVER['REQUEST_METHOD'] == "POST") {
      ...
      if (
         ...
         $check = $database->numRows($database->query("SELECT `tenDangNhap` FROM `tbl_khachhang` WHERE `tenDangNhap`='$tenDangNhap'")) > 0;
         if ($check) {
            $_SESSION['resultCheck'] = "<span style='color: red'>Tên đăng nhập đã tồn tại vui lòng nhập lại</span>";
            header('location: checkout-registration.php?resultCheck=tdn');
         }

         // Kiểm tra email đã tồn tại
         $check1 = $database->numRows($database->query("SELECT `thuDienTuKH` FROM `tbl_khachhang` WHERE `thuDienTuKH`='$email'")) > 0;
         if ($check1) {
            $_SESSION['resultCheck'] = "<span style='color: red'>Email đã tồn tại vui lòng nhập lại</span>";
            header('location: checkout-registration.php?resultCheck=email');
         }

         // Lưu thông tin nếu không có lỗi
         if (!$check && !$check1) {
            $sql = "INSERT INTO `tbl_khachhang`(`maKhachHang`,`tenDangNhap`,`hoTenKhachHang`,`thuDienTuKH`,`matKhau`,`ngaySinh`)
                     VALUES(NULL,'$tenDangNhap','$hoTen','$email','$matKhau','$ngaySinh')";
